...
def edge_combines_two_shapes(edge, shapes):
    shape1 = None
    shape2 = None
    ditsnce_threshold = 6000
    for shape in shapes:
        if get_min_distance(edge[0], shape['shape_strokes'][0]) < ditsnce_threshold:
            shape1 = shape
    for shape in shapes:
        if shape == shape1:
            continue
        if get_min_distance(edge[-1], shape['shape_strokes'][0]) < ditsnce_threshold:
            shape2 = shape
